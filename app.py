...
# ============================================================
    ...
    if len(df) == 0:
        ...
        # I show my safe and trigger foods so I know what the AI
        # is working with before it makes suggestions
        ...
        # --------------------------------------------------------
        # This is the original button that generates 5 meal ideas
        # based on Kiki's logged data. I only call the API when
        # the button is tapped to keep my costs low.
        ...
        # reruns. Every time I interact with the app it reruns from
        # top to bottom — without session_state my chat history would
        # disappear every time I send a message.
        # I initialize an empty list to store my chat messages if
        ...
        if 'chat_history' not in st.session_state:
            # I create an empty list to store my conversation history.
            # Each message is a dict with 'role' and 'content'.
            # 'role' is either 'user' (me) or 'assistant' (the AI).
            st.session_state.chat_history = []

        # I display all previous messages in the chat history.
        # This is what makes it feel like a real chat — I can scroll
        # up and see everything that was said before.
        ...
        # st.chat_input() creates the text box at the bottom of the
        # chat — just like iMessage or WhatsApp. It returns whatever
        # the user typed when they hit send, or None if nothing yet.
        user_input = st.chat_input('Ask Kiki\'s AI chef anything...')

        if user_input:
            # I sanitize the input before sending to the AI
            # to prevent prompt injection attacks.
            clean_input = sanitize_input(user_input)

            # I add the user message to chat history immediately
            # so it shows up in the chat before the AI responds.
            st.session_state.chat_history.append({
                'role': 'user',
                'content': user_input
            })

            # I show the user message as a chat bubble right away
            with st.chat_message('user'):
                st.write(user_input)

            # I show a spinner while the AI is thinking
            with st.chat_message('assistant'):
                with st.spinner('Thinking...'):
                    try:
                        ...
                        # I build the safe and trigger food context
                        # so the AI always knows Kiki's data even
                        ...
                        # The system prompt for chat is the same as
                        # the suggestions prompt — Kiki's full profile,
                        # preferences, and what she hates. This runs
                        # on every message to keep the AI in character.
...
in the food data. Only discuss food, recipes, and IBS-friendly eating for Kiki."""
                        ...
                        # I build the full conversation history to send
                        # to Claude. This is what makes it remember what
                        # was said earlier in the conversation.
                        # I only keep the last 10 messages to avoid
                        # sending too much data and keeping costs low.
                        ...
                        # I show the AI reply as a chat bubble
                        st.write(reply)
        ...
        # I add a button to clear the chat history if Kiki wants
        # to start a fresh conversation. st.session_state.chat_history
        # gets reset to an empty list when this button is tapped.
